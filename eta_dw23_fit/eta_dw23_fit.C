...
void eta_dw23_fit( const std::string bkg_had_infilename,
  ...
  for(int file_index=0; file_index<11; file_index++) {//read loop over files
    ...
    for(int i=0; i<entries; i++) {//Events loop
      ...
      //if(file_index<=1 && rpc_awayclusters3<0 && rpc_awayclusters3>3)
      //  continue;
...
//*/
  ...
  for(int arm=0; arm<2; arm++) {
    for(int charge=0; charge<2; charge++) {
      ...
      h_sbg_fit_dw23[0][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]= new TH1F(name,name,100,-.1,.1);
      ...
      h_combined_mu_bg_dw23[arm][charge] = new TH1F(name,name,100,-0.1,0.1);
      ...
      *v_nbg = mu_bg_yield;
      ...
      data[arm][charge]->plotOn(frame_dw23[arm][charge], Binning(40), DataError(RooAbsData::SumW2) );
      ...
      data[arm][charge]->plotOn(frame_eta[arm][charge], Binning(30), DataError(RooAbsData::SumW2) );
