...
void eta_dw23_fit( const std::string bkg_had_infilename,
  ...
  int in_count[11][2][2],over_count[11][2][2],under_count[11][2][2];
  ...
  for(int file_index=0; file_index<11; file_index++) {//read loop over files
    ...
    for(int i=0; i<entries; i++) {//Events loop
      ...
      if(Wness >= 0.92 && file_index != 0) {
        ...
        h_target_wness[file_index][arm][charge_index]->Fill(Wness);
      ...
      if(Wness >= 0.92 && file_index != 0 && Rpc3dca<100) {
        ...
        h_sbg_fit_dw23[file_index][arm][charge_index][3][0]->Fill(dw23);
      } else if(Wness >= 0.1 && Wness <= 0.7 && file_index==0) {
        h_sbg_fit_eta[file_index][arm][charge_index][3][0]->Fill(eta);
      }
      ...
      for(int charge=0; charge<2; charge++) {
        for(int i=0; i<4; i++) {
          if(read_fit_hists) {
            ...
            h_dw23_sums[source][arm][charge][i] = new TH1F(name,name,nhistbins_2ddw23,distmin[11],distmax[11]);
...
//*/
  ...
  if(save_sbg_hists) {
    sbg_hists_file = new TFile("temp_sbg_fit_pdfs.root","RECREATE");
  }
  ...
  double eff_scaling_factor = 0.407;
  ...
  double mu_bg_generated_events[9] = {
    6400000, //dy
    193600, //light
    55470000, //onium
    106500, //onlyz
  ...
  double mu_bg_scaling_factor[9] = {
    eff_scaling_factor*mu_bg_dimuon_factor[0]*mu_bg_cross_section[0]/(1000*mu_bg_generated_events[0])*228/1.4, //dy
    eff_scaling_factor*mu_bg_dimuon_factor[1]*mu_bg_cross_section[1]/(1000*mu_bg_generated_events[1])*228/1.4*0, //light
    eff_scaling_factor*mu_bg_dimuon_factor[2]*mu_bg_cross_section[2]/(1000*mu_bg_generated_events[2])*228/1.4, //onium
    eff_scaling_factor*mu_bg_dimuon_factor[3]*mu_bg_cross_section[3]/(1000*mu_bg_generated_events[3])*228/1.4, //onlyz
    eff_scaling_factor*mu_bg_dimuon_factor[4]*mu_bg_cross_section[4]/(1000*mu_bg_generated_events[4])*228/1.4, //openbottom
    eff_scaling_factor*mu_bg_dimuon_factor[5]*mu_bg_cross_section[5]/(1000*mu_bg_generated_events[5])*228/1.4, //opencharm
    eff_scaling_factor*mu_bg_dimuon_factor[6]*mu_bg_cross_section[6]/(1000*mu_bg_generated_events[6])*228/1.4, //whad
    eff_scaling_factor*mu_bg_dimuon_factor[7]*mu_bg_cross_section[7]/(1000*mu_bg_generated_events[7])*228/1.4, //wtau
    eff_scaling_factor*mu_bg_dimuon_factor[8]*mu_bg_cross_section[8]/(1000*mu_bg_generated_events[8])*228/1.4}; //z_sum
  ...
  for(int arm=0; arm<2; arm++) {
    for(int charge=0; charge<2; charge++) {
      RooRealVar * v_eta = new RooRealVar("eta","eta",1.1,2.6);
      RooRealVar * v_dw23 = new RooRealVar("dw23","dw23",-.1,.1);
      RooRealVar * v_wness = new RooRealVar("Wness","Wness",0,1);
      ...
      RooDataSet * data = new RooDataSet("data","data",temp_wness_tree,RooArgSet(*v_eta,*v_dw23,*v_wness));
      ...
      h_dw23_data[arm][charge] = new TH1F(name,name,40,distmin[11],distmax[11]);
      h_dw23_data[arm][charge]->Sumw2();
      sprintf(name,"h_eta_data_arm%d_charge%d",arm,charge);
      h_eta_data[arm][charge] = new TH1F(name,name,40,distmin[3],distmax[3]);
      ...
      TF1 *f_hadbg_dw23_proj = new TF1("f_hadbg_dw23_proj",double_gaus_1d,distmin[11],distmax[11],4);
      ...
      h_sbg_fit_dw23[0][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]= new TH1F(name,name,1000,distmin[11],distmax[11]);
      h_sbg_fit_dw23[0][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]->FillRandom("f_hadbg_dw23_proj",500000);
      ...
      h_combined_mu_bg_dw23[arm][charge] = new TH1F(name,name,30,-0.1,0.1);
      sprintf(name,"h_combined_mu_bg_eta_arm%d_charge%d",arm,charge);
      h_combined_mu_bg_eta[arm][charge] = new TH1F(name,name,60,1.1,2.6);
      ...
      RooDataHist * h_bkg_had_eta = new RooDataHist("h_bkg_had_eta","h_bkg_had_eta",RooArgSet(*v_eta),h_sbg_fit_eta[0][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]);
      RooDataHist * h_bkg_had_dw23 = new RooDataHist("h_bkg_had_dw23","h_bkg_had_eta",RooArgSet(*v_dw23),h_sbg_fit_dw23[0][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]);
      RooDataHist * h_bkg_mu_eta = new RooDataHist("h_bkg_mu_eta","h_bkg_mu_eta",RooArgSet(*v_eta),h_combined_mu_bg_eta[arm][charge]);
      RooDataHist * h_bkg_mu_dw23 = new RooDataHist("h_bkg_mu_dw23","h_bkg_mu_dw23",RooArgSet(*v_dw23),h_combined_mu_bg_dw23[arm][charge]);
      RooDataHist * h_sig_mu_eta = new RooDataHist("h_sig_mu_eta","h_sig_mu_eta",RooArgSet(*v_eta),h_sbg_fit_eta[1][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]);
      RooDataHist * h_sig_mu_dw23 = new RooDataHist("h_sig_mu_dw23","h_sig_mu_dw23",RooArgSet(*v_dw23),h_sbg_fit_dw23[1][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]);

      RooHistPdf * pdf_bkg_had_eta = new RooHistPdf("pdf_bkg_had_eta","pdf_bkg_had_eta",RooArgSet(*v_eta),*h_bkg_had_eta);
      RooHistPdf * pdf_bkg_had_dw23 = new RooHistPdf("pdf_bkg_had_dw23","pdf_bkg_had_eta",RooArgSet(*v_dw23),*h_bkg_had_dw23);
      RooHistPdf * pdf_bkg_mu_eta = new RooHistPdf("pdf_bkg_mu_eta","pdf_bkg_mu_eta",RooArgSet(*v_eta),*h_bkg_mu_eta);
      RooHistPdf * pdf_bkg_mu_dw23 = new RooHistPdf("pdf_bkg_mu_dw23","pdf_bkg_mu_dw23",RooArgSet(*v_dw23),*h_bkg_mu_dw23);
      RooHistPdf * pdf_sig_mu_eta = new RooHistPdf("pdf_sig_mu_eta","pdf_sig_mu_eta",RooArgSet(*v_eta),*h_sig_mu_eta);
      RooHistPdf * pdf_sig_mu_dw23 = new RooHistPdf("pdf_sig_mu_dw23","pdf_sig_mu_dw23",RooArgSet(*v_dw23),*h_sig_mu_dw23);
      ...
      RooRealVar * v_nw = new RooRealVar("v_nw", "W number", 1, 0.0, 2000);
      RooRealVar * v_nbg = new RooRealVar("v_nbg", "BG number", 1, 0.0, 2000);
      RooRealVar * v_nhad = new RooRealVar("v_nhad", "fake number", 1, 0.0, 2000);
      ...
      *v_nbg = mu_bg_yield;
      ...
      //do range fraction
      v_eta->setRange("testrange",distmin[3],distmax[3]);
      v_dw23->setRange("testrange",distmin[11],distmax[11]);//(-0.05+charge*0.04),(0.01+charge*0.04));
      RooAbsReal* sig_mu_range_fraction = pdf_bkg_had->createIntegral(RooArgSet(*v_eta,*v_dw23),NormSet(RooArgSet(*v_eta,*v_dw23)),Range("testrange"));
      std::cout << "\n\ntest: " << " " << sig_mu_range_fraction->getVal() << std::endl;
 
      c_sbg_fits_dw23->cd(charge+arm*2+1);
      RooPlot *frame_dw23 = v_dw23->frame(Title(""));
      data->plotOn(frame_dw23, Binning(20), MarkerSize(0.7) );
      //component_model->plotOn(frame_dw23);
      //component_model->plotOn(frame_dw23,Components(*pdf_sig_mu),LineStyle(kDashed),LineColor(kRed));
      //frame_dw23->SetTitle("Signal/Bkg Fit dw23");
      frame_dw23->Draw();
      
      c_sbg_fits_eta->cd(charge+arm*2+1);
      RooPlot *frame_eta = v_eta->frame(Title(""));
      data->plotOn(frame_eta, DataError(RooAbsData::SumW2) );
      component_model->plotOn(frame_eta);
      component_model->plotOn(frame_eta,Components(*pdf_sig_mu),LineStyle(kDashed),LineColor(kRed));
      frame_eta->SetTitle("Signal/Bkg Fit eta");
      frame_eta->Draw(); 
    }
  }
  ...
  for(int i=0; i<9; i++)
    printf("\n%s: %f",mu_bg_labels[i],mu_bg_scaling_factor[i]);
