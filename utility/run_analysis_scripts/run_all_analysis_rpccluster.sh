...
if [ $starting_point -lt 1 ]; then
  ...
  ${code_root_dir}/rpc_cluster_get_phys_dists/run_code_scripts/condor/data_run13_condor.sh
  echo "Finished submitting normal data condor...`date`"
fi
if [ $starting_point -lt 2 ]; then
  echo "Running ot data condor...`date`"
  ${code_root_dir}/rpc_cluster_get_phys_dists/run_code_scripts/condor/data_run13_ot_condor.sh
...
# /direct/phenix+scratch/danielj/condor/rpc_cluster_get_phys_dists/run13data/trees
...
if [ $starting_point -lt 1 ]; then
  ...
  ${code_root_dir}/utility/combine_files/run_combine_condor_run13_rpccluster.sh
  echo"Finished combining data phys_dists...`date`"
fi

if [ $starting_point -lt 2 ]; then
  echo "hadd_combining OT phys_dist output...`date`"
  ${code_root_dir}/utility/combine_files/run_combine_condor_run13ot_rpccluster.sh
...
#now combine the normal data set with the other trigger data set with the merge_other_triggers_rpccluster.C macro, 
# run by the run_merge_other_triggs_rpccluster.sh script found in:
# /direct/phenix+u/workarea/danielj/cvs_code/offline/analysis/danielj/run13_analysis/utility/combine_files
if [ $starting_point -lt 3 ]; then
  echo "Merging different trigger files...`date`"
  ${code_root_dir}/utility/combine_files/run_merge_other_triggs_rpccluster.sh
...
# similar to before, combine the output of the various simulations
# run by the run_merge_other_triggs.sh script both found in:
# /direct/phenix+u/workarea/danielj/cvs_code/offline/analysis/danielj/run13_analysis/utility/combine_files
if [ $starting_point -lt 3 ]; then
  echo "Combining various simulation's condor output files...`date`"
  ${code_root_dir}/utility/combine_files/run_combine_sims_13_rpccluster.sh
  echo "Finished combining simulations...`date`"
...
# /phenix/spin/phnxsp01/danielj/w_asymmetry_analysis_files/run13/rpc_cluster_phys_dists/run13_data_total_trees_combined_test.root
...
if [ $starting_point -lt 4 ]; then
  ...
  root -b -q ${code_root_dir}/wness_preselection/run_code_scripts/wness_preselection_execute_totaltrig_rpccluster.C\(1,${which_year}\)
  ...
  root -b -q ${code_root_dir}/wness_preselection/run_code_scripts/wness_preselection_muonbkg_totaltrig_execute_rpccluster.C\(1,0\)
  echo "wness calculation 3 of 11 done...`date`"
  root -b -q ${code_root_dir}/wness_preselection/run_code_scripts/wness_preselection_muonbkg_totaltrig_execute_rpccluster.C\(1,1\)
  echo "wness calculation 4 of 11 done...`date`"
  root -b -q ${code_root_dir}/wness_preselection/run_code_scripts/wness_preselection_muonbkg_totaltrig_execute_rpccluster.C\(1,2\)
  echo "wness calculation 5 of 11 done...`date`"
  root -b -q ${code_root_dir}/wness_preselection/run_code_scripts/wness_preselection_muonbkg_totaltrig_execute_rpccluster.C\(1,3\)
  echo "wness calculation 6 of 11 done...`date`"
  root -b -q ${code_root_dir}/wness_preselection/run_code_scripts/wness_preselection_muonbkg_totaltrig_execute_rpccluster.C\(1,4\)
  echo "wness calculation 7 of 11 done...`date`"
  root -b -q ${code_root_dir}/wness_preselection/run_code_scripts/wness_preselection_muonbkg_totaltrig_execute_rpccluster.C\(1,5\)
  echo "wness calculation 8 of 11 done...`date`"
  root -b -q ${code_root_dir}/wness_preselection/run_code_scripts/wness_preselection_muonbkg_totaltrig_execute_rpccluster.C\(1,6\)
  echo "wness calculation 9 of 11 done...`date`"
  root -b -q ${code_root_dir}/wness_preselection/run_code_scripts/wness_preselection_muonbkg_totaltrig_execute_rpccluster.C\(1,7\)
  echo "wness calculation 10 of 11 done...`date`"
  root -b -q ${code_root_dir}/wness_preselection/run_code_scripts/wness_preselection_muonbkg_totaltrig_execute_rpccluster.C\(1,8\)
