...
void sbg_fit( const std::string bkg_had_infilename,
  ...
  for(int file_index=0; file_index<11; file_index++) {//read loop over files
    ...
    for(int i=0; i<entries; i++) {//Events loop
      ...
      /* not needed if cut is made in wness step
      if(do_rpc_cluster_cut) {
        if(rpc_awayclusters3 <0 || rpc_awayclusters3>rpc_cluster_cut) continue;
      }*/
...
//----
  ...
  TFile * dw23_infile;
  if(do_rpc_cluster_cut)
    dw23_infile = new TFile(Form("/direct/phenix+u/workarea/danielj/cvs_code/offline/analysis/danielj/run13_analysis/dw23_chong/output_rpccluster_0-%d.root",rpc_cluster_cut));
  else
    dw23_infile = new TFile("/direct/phenix+u/workarea/danielj/cvs_code/offline/analysis/danielj/run13_analysis/dw23_chong/output_nocuts.root");
  TH1F * h_temp[4][2][2];
  ...
  for(int arm=0; arm<2; arm++) {
    for(int charge=0; charge<2; charge++) {
      ...
      h_sbg_fit_dw23[0][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]= (TH1F*)h_temp[0][arm][charge]->Clone(name); 
    }
  }
  ...
  for(int arm=0; arm<2; arm++) {
    for(int charge=0; charge<2; charge++) {
      ...
      /* not needed if cut is made in wness step
      if(do_rpc_cluster_cut) {
        sprintf(conditions,"%s && rpc_awayclusters3>=0 && rpc_awayclusters3<=%d",conditions,rpc_cluster_cut);
      }*/
