...
void wness_preselection_execute(int which_year, int which_ref_run_index, int run_over_which) {
  ...
  if(run_over_which>10) {
    sprintf(inputtreefilename,"/phenix/spin/phnxsp01/danielj/w_asymmetry_analysis_files/run%d/phys_dists/run%d_data_trees_combined.root",which_year,which_year);
    sprintf(treeoutfilename,"/phenix/spin/phnxsp01/danielj/w_asymmetry_analysis_files/run%d/wness_tree/run%d_data_wness_refrun%d.root",which_year,which_year,ref_run[which_ref_run_index]);
  } else {
    ...
    sprintf(treeoutfilename,"/phenix/spin/phnxsp01/danielj/w_asymmetry_analysis_files/run%d/wness_tree/run%d_simulation_wness_refrun%d.root",which_year,which_year,ref_run[which_ref_run_index]);
  }
  
  sprintf(bkg_treeinfilename,"/phenix/spin/phnxsp01/danielj/w_asymmetry_analysis_files/run%d/phys_dists/run%d_data_trees_combined.root",which_year,which_year);
  sprintf(bkg_histoutfilename,"/phenix/spin/phnxsp01/danielj/w_asymmetry_analysis_files/run%d/phys_dists/run%d_data_hists_combined.root",which_year,which_year);
  sprintf(bkg_wnesshistoutfilename,"/phenix/spin/phnxsp01/danielj/w_asymmetry_analysis_files/run%d/wness_tree/run%d_data_hists_combined.root",which_year,which_year);
