...
void wness_preselection_muonbkg_totaltrig_execute_rpccluster(int which_ref_run_index, int run_over_which) {
  ...
  sprintf(bkg_histoutfilename,"/phenix/spin/phnxsp01/danielj/w_asymmetry_analysis_files/run%d/rpc_cluster_phys_dists/run%d_data_total_hists_combined.root",
      which_year,which_year);
  sprintf(bkg_wnesshistoutfilename,"/phenix/spin/phnxsp01/danielj/w_asymmetry_analysis_files/run%d/rpc_cluster_wness_tree/run%d_data_total_hists_combined.root",
      which_year,which_year);
