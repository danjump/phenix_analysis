...
#include "define_dw23_iteration_hist.h"
...
void dw23_iteration( const std::string bkg_had_infilename,
  ...
  for(int file_index=0; file_index<11; file_index++) {//read loop over files
    ...
    for(int i=0; i<entries; i++) {//Events loop
      ...
      if(Wness >= 0.92) {
        if(file_index == 0) {
          h_sbg_fit_dw23[file_index][arm][charge_index][rpcdca_condition][goodfvtx]->Fill(dw23);
        } else {
      ...
      if(Wness >= 0.92) {
        if(file_index==0) {
          h_sbg_fit_dw23[file_index][arm][charge_index][3][1]->Fill(dw23);
  ...
  if(save_sbg_hists) {
    sbg_hists_file = new TFile("/direct/phenix+u/workarea/danielj/cvs_code/offline/analysis/danielj/run13_analysis/dw23_iteration/output/dw23_iteration_pdfs.root","RECREATE");
  }
  ...
  //calculated s/bg yields from standard fit:
  double sbg_fit_yields[3][4] = {{1476.46,1766.42,1496.34,1988.24},
    {0,0,0,0},
    //{297.22,466.29,367.74,493.20},
    //{.75*297.22,.75*466.29,.75*367.74,.75*493.20},
    //{2*297.22,2*466.29,2*367.74,2*493.20},
    {261.78,243.91,177.94,161.65}};
  ...
  TCanvas * c_dw23_iterations_dw23 = new TCanvas("c_dw23_iterations_dw23","c_dw23_iterations_dw23",1500,1500);
  c_dw23_iterations_dw23->Divide(2,2);
  TCanvas * c_dw23_iterations_eta = new TCanvas("c_dw23_iterations_eta","c_dw23_iterations_eta",1500,1500);
  c_dw23_iterations_eta->Divide(2,2);
  ...
  RooDataHist * h_bkg_had_eta[2][2];
  RooDataHist * h_bkg_had_dw23[2][2];
  RooDataHist * h_bkg_mu_eta[2][2];
  RooDataHist * h_bkg_mu_dw23[2][2];
  RooDataHist * h_sig_mu_eta[2][2];
  RooDataHist * h_sig_mu_dw23[2][2];

  RooHistPdf * pdf_bkg_had_eta[2][2];
  RooHistPdf * pdf_bkg_had_dw23[2][2];
  RooHistPdf * pdf_bkg_mu_eta[2][2];
  RooHistPdf * pdf_bkg_mu_dw23[2][2];
  RooHistPdf * pdf_sig_mu_eta[2][2];
  RooHistPdf * pdf_sig_mu_dw23[2][2];
      
  double mu_bg_yield[2][2];
  ...
  for(int arm=0; arm<2; arm++) {
    for(int charge=0; charge<2; charge++) {
      ...
      for(int process=0; process<9; process++) {
        ...
        mu_bg_yield[arm][charge] +=h_sbg_fit_dw23[2+process][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]->Integral()*mu_bg_scaling_factor[process];
      ...
      TCanvas * test = new TCanvas("test","test",900,600);
      test->Divide(2,1);
      double temp_scale = sbg_fit_yields[1][2*arm+charge]/h_sbg_fit_dw23[1][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]->Integral();
      h_sbg_fit_dw23[1][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]->Scale(temp_scale);
      test->cd(1);
      h_sbg_fit_dw23[1][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]->Draw();
      temp_scale = sbg_fit_yields[0][2*arm+charge]/h_sbg_fit_dw23[0][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]->Integral();
      //h_sbg_fit_dw23[0][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]->Scale(temp_scale);
      test->cd(2);
      h_sbg_fit_dw23[0][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]->DrawCopy();
      h_sbg_fit_dw23[0][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]->Add(h_sbg_fit_dw23[1][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice],-1);
      h_sbg_fit_dw23[0][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]->Add(h_combined_mu_bg_eta[arm][charge],-1);
      h_sbg_fit_dw23[0][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]->SetLineColor(kRed);
      h_sbg_fit_dw23[0][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]->DrawCopy("same");
      
      TCanvas * test2 = new TCanvas("test2","test2",900,600);
      test2->Divide(2,1);
      temp_scale = sbg_fit_yields[1][2*arm+charge]/h_sbg_fit_eta[1][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]->Integral();
      h_sbg_fit_eta[1][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]->Scale(temp_scale);
      test2->cd(1);
      h_sbg_fit_eta[1][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]->Draw();
      //temp_scale = sbg_fit_yields[0][2*arm+charge]/h_sbg_fit_eta[0][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]->Integral();
      //h_sbg_fit_eta[0][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]->Scale(temp_scale);
      test2->cd(2);
      h_sbg_fit_eta[0][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]->DrawCopy();
      h_sbg_fit_eta[0][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]->Add(h_sbg_fit_eta[1][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice],-1);
      h_sbg_fit_eta[0][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]->Add(h_combined_mu_bg_eta[arm][charge],-1);
      h_sbg_fit_eta[0][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]->SetLineColor(kRed);
      h_sbg_fit_eta[0][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]->DrawCopy("same");
      
      
      //set up s/bg extraction pdfs
      sprintf(name,"h_bkg_had_eta_arm%d_charge%d",arm,charge);
      h_bkg_had_eta[arm][charge] = new RooDataHist(name,name,RooArgSet(*v_eta[arm][charge]),h_sbg_fit_eta[0][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]);
      sprintf(name,"h_bkg_had_dw23_arm%d_charge%d",arm,charge);
      h_bkg_had_dw23[arm][charge] = new RooDataHist(name,name,RooArgSet(*v_dw23[arm][charge]),h_sbg_fit_dw23[0][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]);
      sprintf(name,"h_bkg_mu_eta_arm%d_charge%d",arm,charge);
      h_bkg_mu_eta[arm][charge] = new RooDataHist(name,name,RooArgSet(*v_eta[arm][charge]),h_combined_mu_bg_eta[arm][charge]);
      sprintf(name,"h_bkg_mu_dw23_arm%d_charge%d",arm,charge);
      h_bkg_mu_dw23[arm][charge] = new RooDataHist(name,name,RooArgSet(*v_dw23[arm][charge]),h_combined_mu_bg_dw23[arm][charge]);
      sprintf(name,"h_sig_mu_eta_arm%d_charge%d",arm,charge);
      h_sig_mu_eta[arm][charge] = new RooDataHist(name,name,RooArgSet(*v_eta[arm][charge]),h_sbg_fit_eta[1][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]);
      sprintf(name,"h_sig_mu_dw23_arm%d_charge%d",arm,charge);
      h_sig_mu_dw23[arm][charge] = new RooDataHist(name,name,RooArgSet(*v_dw23[arm][charge]),h_sbg_fit_dw23[1][arm][charge][sbg_rpcdca_choice][sbg_fvtx_choice]);
//---
      sprintf(name,"pdf_bkg_had_eta_arm%d_charge%d",arm,charge);
      pdf_bkg_had_eta[arm][charge] = new RooHistPdf(name,name,RooArgSet(*v_eta[arm][charge]),*h_bkg_had_eta[arm][charge]);
      sprintf(name,"pdf_bkg_had_dw23_arm%d_charge%d",arm,charge);
      pdf_bkg_had_dw23[arm][charge] = new RooHistPdf(name,name,RooArgSet(*v_dw23[arm][charge]),*h_bkg_had_dw23[arm][charge]);
      sprintf(name,"pdf_bkg_mu_eta_arm%d_charge%d",arm,charge);
      pdf_bkg_mu_eta[arm][charge] = new RooHistPdf(name,name,RooArgSet(*v_eta[arm][charge]),*h_bkg_mu_eta[arm][charge]);
      sprintf(name,"pdf_bkg_mu_dw23_arm%d_charge%d",arm,charge);
      pdf_bkg_mu_dw23[arm][charge] = new RooHistPdf(name,name,RooArgSet(*v_dw23[arm][charge]),*h_bkg_mu_dw23[arm][charge]);
      sprintf(name,"pdf_sig_mu_eta_arm%d_charge%d",arm,charge);
      pdf_sig_mu_eta[arm][charge] = new RooHistPdf(name,name,RooArgSet(*v_eta[arm][charge]),*h_sig_mu_eta[arm][charge]);
      sprintf(name,"pdf_sig_mu_dw23_arm%d_charge%d",arm,charge);
      pdf_sig_mu_dw23[arm][charge] = new RooHistPdf(name,name,RooArgSet(*v_dw23[arm][charge]),*h_sig_mu_dw23[arm][charge]);
      ...
      if(save_sbg_hists) {
        ...
        pdf_sig_mu_eta[arm][charge]->fillHistogram(h_sbg_pdf_sig_mu_eta[arm][charge],RooArgList(*v_eta[arm][charge]));
        ...
        pdf_sig_mu_dw23[arm][charge]->fillHistogram(h_sbg_pdf_sig_mu_dw23[arm][charge],RooArgList(*v_dw23[arm][charge]));
        ...
        pdf_bkg_had_eta[arm][charge]->fillHistogram(h_sbg_pdf_bkg_had_eta[arm][charge],RooArgList(*v_eta[arm][charge]));
        ...
        pdf_bkg_had_dw23[arm][charge]->fillHistogram(h_sbg_pdf_bkg_had_dw23[arm][charge],RooArgList(*v_dw23[arm][charge]));
        ...
        pdf_bkg_mu_eta[arm][charge]->fillHistogram(h_sbg_pdf_bkg_mu_eta[arm][charge],RooArgList(*v_eta[arm][charge]));
        ...
        pdf_bkg_mu_dw23[arm][charge]->fillHistogram(h_sbg_pdf_bkg_mu_dw23[arm][charge],RooArgList(*v_dw23[arm][charge]));
  ...
  for(int arm = 0; arm<2; arm++) {
    for(int charge=0; charge<2; charge++) {
      RooProdPdf * pdf_bkg_had = new RooProdPdf("pdf_bkg_had","pdf_bkg_had",RooArgList(*pdf_bkg_had_eta[arm][charge],*pdf_bkg_had_dw23[arm][charge]),0);
      RooProdPdf * pdf_bkg_mu = new RooProdPdf("pdf_bkg_mu","pdf_bkg_mu",RooArgList(*pdf_bkg_mu_eta[arm][charge],*pdf_bkg_mu_dw23[arm][charge]),0);
      RooProdPdf * pdf_sig_mu = new RooProdPdf("pdf_sig_mu","pdf_sig_mu",RooArgList(*pdf_sig_mu_eta[arm][charge],*pdf_sig_mu_dw23[arm][charge]),0);
      ...
      *v_nbg = mu_bg_yield[arm][charge];
      //*v_nbg = 30;

      printf("\n\nmuonic bg yields: %f\n\n",mu_bg_yield[arm][charge]);
      ...
      c_dw23_iterations_dw23->cd(charge+arm*2+1);
      ...
      c_dw23_iterations_eta->cd(charge+arm*2+1);
  ...
  ofstream smout;
  smout.open("/direct/phenix+u/workarea/danielj/cvs_code/offline/analysis/danielj/run13_analysis/dw23_iteration/output/sig_mu_yields.txt");
  ...
  ofstream bhout;
  bhout.open("/direct/phenix+u/workarea/danielj/cvs_code/offline/analysis/danielj/run13_analysis/dw23_iteration/output/bkg_had_yields.txt");
  ...
  ofstream bmout;
  bmout.open("/direct/phenix+u/workarea/danielj/cvs_code/offline/analysis/danielj/run13_analysis/dw23_iteration/output/bkg_mu_scaling_yields.txt");
  ...
  sbg_outfile.open("/direct/phenix+u/workarea/danielj/cvs_code/offline/analysis/danielj/run13_analysis/dw23_iteration/output/sbg_values.txt");
  ...
  for(int arm=0; arm<2; arm++) {
    for(int charge=0; charge<2; charge++) {
      ...
      c_dw23_iteration_dw23->cd(charge+2*arm+1);
      ...
      c_dw23_iteration_eta->cd(charge+2*arm+1);
